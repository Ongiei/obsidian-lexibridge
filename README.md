...
用户主动查词或创建笔记时，若 ECDICT 没有收录，可选择使用有道网页 JSON 接口补充音标、释义、词形、网络释义和例句。查词侧边栏可在 ECDICT 与有道之间切换；单词文件的右键菜单也提供按需增强入口。还可以在设置中选择是否让编辑器选词右键菜单显示有道在线查询。
...
- 在阅读模式和 Live Preview 中显示不写入 Markdown 的虚拟链接；悬浮时使用 Obsidian Page Preview，点击窗口直接渲染对应单词笔记，并可继续查词、创建词元笔记或转为真实链接；源码模式不添加装饰。
